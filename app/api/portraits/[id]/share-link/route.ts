...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const returnedOwner = await portraitReturnAccess(id);
  if (!userId && !sessionId && !returnedOwner) return NextResponse.json({ error: "Sign in or open your original preview" }, { status: 401 });
  ...
  if (!portrait || !((userId && portrait.userId === userId) || (sessionId && portrait.sessionId === sessionId) || returnedOwner)) {
