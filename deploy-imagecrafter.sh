...
PORTRAIT_WATERMARK_OPACITY="${PORTRAIT_WATERMARK_OPACITY:-0.15}"
PORTRAIT_PREVIEW_MAX_RESOLUTION="${PORTRAIT_PREVIEW_MAX_RESOLUTION:-1024}"
PORTRAIT_HIRES_RESOLUTION="${PORTRAIT_HIRES_RESOLUTION:-4096}"
PORTRAIT_MAX_UPLOAD_SIZE_MB="${PORTRAIT_MAX_UPLOAD_SIZE_MB:-10}"
PORTRAIT_DOWNLOAD_EXPIRY_HOURS="${PORTRAIT_DOWNLOAD_EXPIRY_HOURS:-72}"
PORTRAIT_MAX_DOWNLOADS="${PORTRAIT_MAX_DOWNLOADS:-5}"

# Prodigi Print-on-Demand — Phase 4
PRODIGI_API_KEY="${PRODIGI_API_KEY:-}"
PRODIGI_SANDBOX_API_KEY="${PRODIGI_SANDBOX_API_KEY:-$PRODIGI_API_KEY}"
PRODIGI_API_URL="${PRODIGI_API_URL:-https://api.prodigi.com/v4.0}"
PRODIGI_SANDBOX_URL="${PRODIGI_SANDBOX_URL:-https://api.sandbox.prodigi.com/v4.0}"
PRODIGI_WEBHOOK_SECRET="${PRODIGI_WEBHOOK_SECRET:-}"
USE_PRODIGI_SANDBOX="${USE_PRODIGI_SANDBOX:-true}"

# Email — Brevo SMTP
SMTP_SERVER="${SMTP_SERVER:-smtp-relay.brevo.com}"
SMTP_PORT="${SMTP_PORT:-587}"
LOGIN="${LOGIN:-}"
BREVO_PAYLOAD_SMTP_API_KEY="${BREVO_PAYLOAD_SMTP_API_KEY:-}"
EMAIL_FROM="${EMAIL_FROM:-ImageCrafter <hello@imagecrafter.app>}"
...
if [[ "$SKIP_DEPLOY" == false ]]; then
  ...
  if [[ "$STACK_DEPLOY" == true ]]; then
    ...
    export PORTRAIT_WATERMARK_OPACITY PORTRAIT_PREVIEW_MAX_RESOLUTION PORTRAIT_HIRES_RESOLUTION
    export PORTRAIT_MAX_UPLOAD_SIZE_MB PORTRAIT_DOWNLOAD_EXPIRY_HOURS PORTRAIT_MAX_DOWNLOADS
